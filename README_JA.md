...
バージョン 0.2.0（公開前） ／ Tokumaru Labs ／ Chrome 拡張機能（Manifest V3）

## 表示モードと更新後のデータ

歯車 → **DISPLAY MODE** で POPUP / FLOATING / SIDE PANEL を選び、
**OPEN SELECTED VIEW** で開きます。次回のツールバーアイコンも選んだ方式で開きます。
自由配置はヘッダーをドラッグして移動し、◇で固定、↔でサイズ切替、×で閉じます。
ヘッダーにフォーカスすると矢印キーでも移動できます。位置は端末内に記憶します。

自由配置はユーザーが拡張アイコンを押した通常のWebページにだけ挿入します。
別サイトへ移った場合は再度アイコンを押してください。ブラウザ設定画面等では使えません。
サイドパネルはタブをまたいで表示でき、左右位置はブラウザ設定に従います。
Chrome 116以降が対象です。Brave等の派生ブラウザは別途実機確認が必要です。

同じ拡張IDの通常更新では、統計・カウント・タイマー・設定を引き継ぎます。
0.2.0は旧データを初期化せず、更新前データを別キーに一度保存します。
以前の約400日超の自動間引きも止めました。既に削除された履歴の復元はできません。
アンインストールと確認付きCLEAR ALL DATAは記録の削除操作です。
既存利用者は削除・再インストールせず、同じ拡張の更新を使ってください。

更新後、最初に表示したメイン画面のCOUNT下に **NEW FEATURE** が一度だけ出ます。
テーマに合わせた短い走査線演出のあと、読める静止表示になり、×で閉じられます。
新規インストールでは表示しません。既読情報は統計と別に保存します。
...
設定画面から **ORIGINAL**、**ARCADE**、**EDITORIAL** の3種類を選択できます。
変わるのは見た目だけで、タイマー、カウンター、統計、リセット、節目演出のロジックは
すべて共通です。選択したテーマは端末内に保存され、次に開いたときも維持されます。
ポップアップ上部には現在のローカル時刻も任意で小さく表示できます。
...
現在のソースをChromeへ手動で読み込む手順は次のとおりです。
...
```
npm test              # popup統合経路を含む既存機能・更新保持・並行操作・一度だけ案内のテスト
npm run verify        # 公開前チェック（manifest、ロケール、権限、アイコン、リモートコード不使用）
npm run package       # Chrome ウェブストア用 ZIP を dist/ に作成
npm run prepare-update # 画像生成、テスト、検証、ZIP作成を一括実行
npm run submission-kit # 提出用ZIP・画像・文面を dist/ の1フォルダに集約
```
...
- **権限：** `storage`、`activeTab`、`scripting`、`sidePanel`
- 常時のhost_permissionsは要求しません。自由配置時だけUIを挿入し、サービスワーカーで保存を一元化します
